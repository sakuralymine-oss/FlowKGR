...
class InductiveDataLoader:
    ...
    def __init__(
        ...
        if self.ppr_iterations < 1:
            raise ValueError("ppr_iterations must be positive")
        if not os.path.isdir(self.ind_dir):
            raise FileNotFoundError(
                f"inductive graph directory not found: {self.ind_dir}"
            )

        self.entity2id = self._read_id_file(
            os.path.join(self.task_dir, "entities.txt")
        )
        self.entity2id_ind = self._read_id_file(
            os.path.join(self.ind_dir, "entities.txt")
        )
        self.relation2id = self._read_id_file(
            os.path.join(self.task_dir, "relations.txt")
        )
        self.n_ent = len(self.entity2id)
        ...
        self.id2entity = {idx: name for name, idx in self.entity2id.items()}
        self.id2entity_ind = {
            idx: name for name, idx in self.entity2id_ind.items()
        }
        self.id2relation = {idx: name for name, idx in self.relation2id.items()}

        overlap = set(self.entity2id).intersection(self.entity2id_ind)
        if overlap:
            preview = ", ".join(sorted(overlap)[:5])
            warnings.warn(
                f"train/inductive entity files share {len(overlap)} surface names; "
                f"they remain separate local IDs, e.g. {preview}"
            )

        tra_train_forward = self._read_forward_triples(
            self.task_dir, "train.txt", self.entity2id
        )
        tra_valid_forward = self._read_forward_triples(
            self.task_dir, "valid.txt", self.entity2id
        )
        tra_test_forward = self._read_forward_triples(
            self.task_dir, "test.txt", self.entity2id
        )
        ind_train_forward = self._read_forward_triples(
            self.ind_dir, "train.txt", self.entity2id_ind
        )
        ind_valid_forward = self._read_forward_triples(
            self.ind_dir, "valid.txt", self.entity2id_ind
        )
        ind_test_forward = self._read_forward_triples(
            self.ind_dir, "test.txt", self.entity2id_ind
        )

        
        self.train_graph_triples = self.double_triples(tra_train_forward)
        self.ind_graph_triples = self.double_triples(ind_train_forward)
        self.train_KG, self.train_sub = self.load_graph(
            self.train_graph_triples, self.n_ent
        )
        self.ind_KG, self.ind_sub = self.load_graph(
            self.ind_graph_triples, self.n_ent_ind
        )
        
        
        
        
        self.train_relation_profile = self._build_relation_profile(
            self.train_graph_triples, self.n_ent
        )
        self.ind_relation_profile = self._build_relation_profile(
            self.ind_graph_triples, self.n_ent_ind
        )
        self.train_transition = self._build_transition_matrix(
            self.train_KG, self.n_ent
        )
        self.ind_transition = self._build_transition_matrix(
            self.ind_KG, self.n_ent_ind
        )
        self._ppr_cache_train = {}
        self._ppr_cache_eval = {}
        ...
        self.train_triples = self.double_triples(tra_valid_forward)
        self.valid_triples = self.double_triples(tra_test_forward)
        self.test_triples = self.double_triples(
            ind_valid_forward + ind_test_forward
        )
        self.train_q, self.train_a = self.load_query(self.train_triples)
        self.valid_q, self.valid_a = self.load_query(self.valid_triples)
        self.test_q, self.test_a = self.load_query(self.test_triples)
        ...
        self.val_filters = self.get_filter(
            self.double_triples(
                tra_train_forward + tra_valid_forward + tra_test_forward
            )
        )
        self.tst_filters = self.get_filter(
            self.double_triples(
                ind_train_forward + ind_valid_forward + ind_test_forward
            )
        )
        ...
        with open(path, "r", encoding="utf-8") as handle:
            for line in handle:
                ...
                if not parts:
                    continue
                mapping[parts[0]] = (
                    int(parts[1]) if len(parts) > 1 else len(mapping)
                )
        ids = sorted(mapping.values())
        if ids != list(range(len(ids))):
            raise ValueError(f"IDs in {path} must be contiguous from zero")
    ...
    def _read_forward_triples(
        self, directory: str, filename: str, entity2id: Dict[str, int]
    ) -> List[List[int]]:
        triples = []
        ...
        with open(path, "r", encoding="utf-8") as handle:
            for line in handle:
                ...
                if relation not in self.relation2id:
                    raise KeyError(
                        f"relation {relation!r} from {path} is absent from "
                        f"{os.path.join(self.task_dir, 'relations.txt')}"
                    )
                triples.append(
                    [
                        entity2id[head],
    ...
    def entity_count(self, mode: str) -> int:
        if mode in {"train", "valid"}:
            return self.n_ent
        if mode in {"test", "eval", "inductive"}:
            return self.n_ent_ind
        raise ValueError(f"unknown mode: {mode}")

    def _graph_for_mode(self, mode: str):
        if mode in {"train", "valid"}:
            return (
                self.train_KG,
                self.train_sub,
    ...
    def load_graph(
        self, triples: List[List[int]], n_ent: int
        ...
        identity = np.column_stack(
            (
                ...
                np.full(n_ent, self_loop_rel, dtype=np.int64),
                np.arange(n_ent, dtype=np.int64),
            )
        )
        kg = np.concatenate((kg, identity), axis=0)
        ...
        subject_matrix = csr_matrix(
            (np.ones(edge_count), (np.arange(edge_count), kg[:, 0])),
            shape=(edge_count, n_ent),
        )
        return kg, subject_matrix

    def _build_transition_matrix(
        self, kg: np.ndarray, n_ent: int
    ) -> csr_matrix:
        non_self_loop = kg[:, 1] != (2 * self.n_rel)
        heads = kg[non_self_loop, 0]
        tails = kg[non_self_loop, 2]
    ...
    def _build_relation_profile(
        self, triples: List[List[int]], n_ent: int
    ) -> csr_matrix:
        triples = np.asarray(triples, dtype=np.int64).reshape(-1, 3)
        if triples.size == 0:
            return csr_matrix(
                (n_ent, 2 * self.n_rel), dtype=np.float32
            )
        profile = csr_matrix(
            (
                np.ones(triples.shape[0], dtype=np.float32),
                (triples[:, 0], triples[:, 1]),
            ),
            shape=(n_ent, 2 * self.n_rel),
            dtype=np.float32,
        )
        return profile.tocsr()

    def get_candidate_relation_profile(
        self, nodes: np.ndarray, mode: str = "train"
    ) -> torch.Tensor:
        
        nodes = np.asarray(nodes, dtype=np.int64)
        if mode in {"train", "valid"}:
            profile = self.train_relation_profile
        elif mode in {"test", "eval", "inductive"}:
            profile = self.ind_relation_profile
        else:
            raise ValueError(f"unknown mode: {mode}")
        counts = profile[nodes[:, 1]].toarray().astype(
            np.float32, copy=False
        )
        device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
        return torch.as_tensor(counts, dtype=torch.float32, device=device)

    @staticmethod
    def _resolve_subgraph_size_for_n(topk: float, n_ent: int) -> int:
        topk = float(topk)
        if topk <= 0:
            raise ValueError("one-shot PPR topk must be positive")
        size = int(np.ceil(topk * n_ent)) if topk < 1.0 else int(topk)
        return int(min(max(size, 1), n_ent))

    def _resolve_subgraph_size(self, topk: float, mode: str = "train") -> int:
        return self._resolve_subgraph_size_for_n(
            topk, self.entity_count(mode)
        )

    def _ppr_scores(self, root: int, mode: str) -> np.ndarray:
        _, _, transition, cache, n_ent = self._graph_for_mode(mode)
        root = int(root)
        if root in cache:
            return cache[root]
        teleport = np.zeros(n_ent, dtype=np.float32)
        teleport[root] = 1.0
        scores = teleport.copy()
        for _ in range(self.ppr_iterations):
            scores = (
    ...
    def get_ppr_subgraph(
        ...
        n_ent = self.entity_count(mode)
        k = self._resolve_subgraph_size_for_n(topk, n_ent)
        rows = []
        for row, root in enumerate(np.asarray(subs, dtype=np.int64)):
            scores = self._ppr_scores(int(root), mode)
            if k >= n_ent:
                entities = np.arange(n_ent, dtype=np.int64)
            else:
                entities = np.argpartition(-scores, kth=k - 1)[:k].astype(
                    np.int64, copy=False
                )
    ...
    def get_subgraph_edges(
        ...
        kg, subject_matrix, _, _, n_ent = self._graph_for_mode(mode)
        nodes = np.asarray(nodes, dtype=np.int64)
        node_lookup = np.full((batch_size, n_ent), -1, dtype=np.int64)
        node_lookup[nodes[:, 0], nodes[:, 1]] = np.arange(
            nodes.shape[0], dtype=np.int64
        )
        node_1hot = csr_matrix(
            (np.ones(len(nodes)), (nodes[:, 1], nodes[:, 0])),
            shape=(n_ent, batch_size),
        )
        edge_1hot = subject_matrix.dot(node_1hot)
        edge_ids, active_node_ids = np.nonzero(edge_1hot)
        selected = np.concatenate(
    ...
    def load_query(
        ...
        queries = list(grouped.keys())
        answers = [
            np.asarray(sorted(grouped[q]), dtype=np.int64) for q in queries
        ]
        return queries, answers
    ...
    def get_batch(self, batch_idx: np.ndarray, data: str = "train"):
        ...
        subs = np.asarray([query[0] for query in selected], dtype=np.int64)
        rels = np.asarray([query[1] for query in selected], dtype=np.int64)
        labels = np.zeros((len(batch_idx), n_ent), dtype=np.float32)
        for row, index in enumerate(batch_idx):
            labels[row, answers[int(index)]] = 1.0
        return subs, rels, labels
    ...
    def shuffle_train(self):
        if self.n_train <= 1:
            return
        order = np.random.permutation(self.n_train)
        self.train_q = [self.train_q[int(index)] for index in order]
        self.train_a = [self.train_a[int(index)] for index in order]
