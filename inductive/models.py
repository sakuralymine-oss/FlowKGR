...
class FMGNNReasoner(nn.Module):
    ...
    def __init__(self, params, loader):
        ...
        self.inductive = bool(getattr(params, "inductive", False))
        self.local_relation_context = bool(
            getattr(params, "local_relation_context", False)
        )
        ...
        if self.local_relation_context:
            self.local_relation_encoder = nn.Sequential(
                nn.Linear(2 * self.n_rel, self.hidden_dim, bias=False),
                nn.LayerNorm(self.hidden_dim),
                nn.SiLU(),
                nn.Linear(self.hidden_dim, self.hidden_dim, bias=False),
            )
            self.local_relation_gate = nn.Sequential(
                nn.Linear(self.hidden_dim * 3, self.hidden_dim),
                nn.SiLU(),
            ...
            nn.init.zeros_(self.local_relation_gate[-1].weight)
            nn.init.constant_(self.local_relation_gate[-1].bias, -2.0)
    ...
    def _entity_count(self, mode: str) -> int:
        if hasattr(self.loader, "entity_count"):
            return int(self.loader.entity_count(mode))
        return int(self.loader.n_ent)

    def _candidate_entity_state(
        self, nodes: torch.Tensor, hidden: torch.Tensor
    ) -> torch.Tensor:
        
        
        if self.inductive:
            return torch.zeros_like(hidden)
        return self.entity_state(nodes[:, 1])
    ...
    def _dense_from_node_logits(
        ...
        batch_size: int,
        n_ent: Optional[int] = None,
    ) -> torch.Tensor:
        if n_ent is None:
            n_ent = int(self.loader.n_ent)
        scores = torch.full(
            (batch_size, int(n_ent)),
    ...
    def _add_local_relation_context(
        self,
        q_rel: torch.Tensor,
        nodes: torch.Tensor,
        hidden: torch.Tensor,
        ...
        if not self.local_relation_context:
            return hidden
        profile = self.loader.get_candidate_relation_profile(
            nodes.detach().cpu().numpy(), mode
        ).to(device=hidden.device, dtype=hidden.dtype)
        
        
        local = self.local_relation_encoder(torch.log1p(profile))
        query = self.rela_embed(q_rel)[nodes[:, 0]]
        gate = torch.sigmoid(
            self.local_relation_gate(
                torch.cat([hidden, local, local * query], dim=-1)
            )
        )
        return hidden + gate * local
    ...
    def forward(
        ...
        for path_index in range(path_samples):
            ...
            final_score_paths.append(self._node_scores(nodes, x1, batch_size, mode))
    ...
    def contrastive_loss(
        ...
        hard_count = int(round(count * ratio))
        random_count = count - hard_count
        selected_mask = torch.zeros_like(row_negative)
        selected_parts = []
        ...
        if hard_count > 0:
            ...
            hard_index = torch.topk(
                hard_scores, k=hard_count, dim=1
            ).indices
            selected_parts.append(hard_index)
            selected_mask.scatter_(1, hard_index, True)

        if random_count > 0:
            remaining_negative = row_negative & ~selected_mask
            random_priority = torch.rand_like(row_scores).masked_fill(
                ~remaining_negative, float("-inf")
            )
            random_index = torch.topk(
                random_priority, k=random_count, dim=1
            ).indices
            selected_parts.append(random_index)

        negative_index = torch.cat(selected_parts, dim=1)
    ...
    def top10_boundary_loss(
        ...
        valid_rows = positive.any(dim=1) & (negative_count >= 10)
        if not torch.any(valid_rows):
            return scores.sum() * 0.0

        negative_scores = scores.detach().masked_fill(
            ~negative, float("-inf")
        )
        tenth_index = torch.topk(
            negative_scores, k=10, dim=1
        ).indices[:, 9]
        tenth_negative = scores.gather(1, tenth_index.unsqueeze(1)).squeeze(1)

        
        
        
        outside_top10 = positive & valid_rows.unsqueeze(1) & (
            scores.detach() <= tenth_negative.unsqueeze(1)
        )
        if not torch.any(outside_top10):
            return scores.sum() * 0.0

        boundary_gap = (
            tenth_negative.unsqueeze(1) - scores + float(margin)
        )
        return F.softplus(boundary_gap[outside_top10]).mean()
    ...
    def compute_losses(
        ...
        contrastive = self.contrastive_loss(
            ...
            float(weights.get("hard_negative_ratio", 0.0)),
        )
        top10 = self.top10_boundary_loss(
            state.scores,
            labels,
            state.candidate_mask,
            float(weights.get("top10_margin", 0.10)),
        )
        vae_rec = (
